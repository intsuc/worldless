...
storage, and function macros. All state is VM-owned logical data; the pack does
not observe or modify a physical Minecraft world.
...
logical VM state, not Minecraft players or entities.
...
threshold. The measurements describe Worldless on this machine, not a
Minecraft server, and larger workloads must be measured against their caller's
own command limit.
