execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[0]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[0] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[1]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[1] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[2]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[2] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[3]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[3] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[4]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[4] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[5]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[5] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[6]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[6] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[7]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[7] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[8]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[8] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[9]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[9] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[10]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[10] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[11]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[11] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[12]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[12] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[13]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[13] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[14]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[14] int 1 run scoreboard players get #value scalar_replace
execute store result score #value scalar_replace run data get storage scalar_replacement:state work.values[15]
scoreboard players operation #value scalar_replace *= #factor scalar_replace
scoreboard players add #value scalar_replace 7
execute store result storage scalar_replacement:state work.values[15] int 1 run scoreboard players get #value scalar_replace
