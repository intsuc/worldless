...
fn function_conditions_test_the_return_value_without_using_success() {
    ...
    assert_eq!(
        vm.execute_function("example:if_nonzero", LIMIT).unwrap(),
        returned(true, 11)
    );
    assert_eq!(
    ...
    assert_eq!(
        vm.execute_function("example:unless_nonzero", LIMIT)
            .unwrap(),
        returned(false, 0)
    );
    assert_eq!(
    ...
    assert_eq!(
        vm.execute_function("example:unless_failure", LIMIT)
            .unwrap(),
        returned(true, 13)
    );
    assert_eq!(
        vm.execute_function("example:unless_fallthrough", LIMIT)
            .unwrap(),
        returned(true, 14)
    );
    assert_eq!(
...
fn only_a_return_from_the_tested_function_produces_a_condition_value() {
    ...
    assert_eq!(
        vm.execute_function("example:normal_is_zero", LIMIT)
            .unwrap(),
        returned(true, 1)
    );
    assert_eq!(
        vm.execute_function("example:returning_is_nonzero", LIMIT)
            .unwrap(),
        returned(true, 2)
    );
    assert_eq!(
        vm.execute_function("example:no_callback_is_not_zero", LIMIT)
            .unwrap(),
        returned(false, 0)
    );
}
...
fn unknown_condition_functions_abort_the_chain_for_both_polarities() {
    ...
    assert_eq!(
        vm.execute_function("example:if_before_return", LIMIT)
            .unwrap(),
        returned(true, 6)
    );
    assert_eq!(
        vm.execute_function("example:inactive_after_return", LIMIT)
            .unwrap(),
        FunctionOutcome::FellThrough
    );
    assert_eq!(
        vm.execute_function("example:store_before_unknown", LIMIT)
            .unwrap(),
        returned(true, 9)
    );
}
...
fn tested_functions_keep_side_effects_but_do_not_receive_outer_stores() {
    ...
    vm.execute_function("example:setup", LIMIT).unwrap();
    assert_eq!(
        vm.execute_function("example:filtered_store", LIMIT)
            .unwrap(),
        returned(true, 9)
    );
    assert_eq!(
        vm.execute_function("example:read_evaluations", LIMIT)
            .unwrap(),
        returned(true, 1)
    );
    assert_eq!(
...
fn function_conditions_resume_the_remaining_modifier_chain_in_order() {
    ...
    vm.execute_function("example:setup", LIMIT).unwrap();
    assert_eq!(
        vm.execute_function("example:function_then_score", LIMIT)
            .unwrap(),
        returned(true, 8)
    );
    vm.execute_function("example:reset_calls", LIMIT).unwrap();
    assert_eq!(
        vm.execute_function("example:score_then_function", LIMIT)
            .unwrap(),
        returned(true, 9)
    );
    vm.execute_function("example:reset_calls", LIMIT).unwrap();
    assert_eq!(
        vm.execute_function("example:inactive_function", LIMIT)
            .unwrap(),
        returned(true, 0)
    );
    assert_eq!(
        vm.execute_function("example:two_function_conditions", LIMIT)
            .unwrap(),
        returned(true, 11)
    );
    assert_eq!(
    ...
    vm.execute_function("example:reset_calls", LIMIT).unwrap();
    assert_eq!(
        vm.execute_function("example:short_circuited_function", LIMIT)
            .unwrap(),
        returned(true, 0)
    );
}
...
fn quota_failure_preserves_tested_function_side_effects() {
    ...
    vm.execute_function("example:setup", LIMIT).unwrap();
    assert_eq!(
        vm.execute_function("example:condition", 3),
